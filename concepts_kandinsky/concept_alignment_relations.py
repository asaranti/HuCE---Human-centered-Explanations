"""
    Concept for symmetric and non-symmetric alignment relations
...
from concepts_kandinsky.kandinsky_concepts_utils.geometric_relations_utils import point_contained_alignment_triangles
...
# [0.] x-axis relation =================================================================================================
########################################################################################################################
def alignment_relations(all_node_ids: list, nodes_shapes: list, nodes_colors: list,
        ...
        for nodes_with_positions_combinations in all_nodes_with_positions_combinations:
            ...
            for graph_idx in range(len(all_graphs)):
                ...
                for node_idx in list(graph.nodes):
                    ...
                    sub_concept_str += new_line_or_not + f"contains(graph_{graph_idx_over_all}, " \
                                                         f"object_{graph_idx_over_all}_{node_idx}), "

                    # [4.4.] Alignment relations -----------------------------------------------------------------------
                    for rest_node_idx in rest_of_nodes_indexes_list:
                        ...
                        if (node_idx, rest_node_idx) in graph_edges_list or \
                            ...
                            # Compute the alignment relation for the object from [left ---> right] ---------------------
                            alignment_relation_1 = point_contained_alignment_triangles(
                                image_x_left_lim,
                                image_x_right_lim,
                                image_y_down_lim,
                                image_y_up_lim,
                                nodes_with_noise_pos_dict[node_idx][0],         # Center object ~~~~~~~~~~~~~~~~~~~~~~~~
                                nodes_with_noise_pos_dict[node_idx][1],
                                nodes_with_noise_pos_dict[rest_node_idx][0],    # Other object ~~~~~~~~~~~~~~~~~~~~~~~~~
                                nodes_with_noise_pos_dict[rest_node_idx][1]
                            )

                            sub_concept_str += f"{alignment_relation_1}(object_{graph_idx_over_all}_{node_idx}, " \
                                               f"object_{graph_idx_over_all}_{rest_node_idx}), " \
                                               + new_line_or_not

                            # Compute the alignment relation for the object from [right ---> left] ---------------------
                            alignment_relation_2 = point_contained_alignment_triangles(
                                image_x_left_lim,
                                image_x_right_lim,
                                image_y_down_lim,
                                image_y_up_lim,
                                nodes_with_noise_pos_dict[rest_node_idx][0],    # Center object ~~~~~~~~~~~~~~~~~~~~~~~~
                                nodes_with_noise_pos_dict[rest_node_idx][1],
                                nodes_with_noise_pos_dict[node_idx][0],         # Other object ~~~~~~~~~~~~~~~~~~~~~~~~~
                                nodes_with_noise_pos_dict[node_idx][1]
                            )

                            sub_concept_str += f"{alignment_relation_2}(object_{graph_idx_over_all}_{rest_node_idx}, " \
                                               f"object_{graph_idx_over_all}_{node_idx}), " \
                                               + new_line_or_not

                            print(f">>>>>>>>>>>>>>>>>>>>>>>>>>> EDGE: {(node_idx, rest_node_idx)}, "
                                  f"{alignment_relation_1}, {alignment_relation_2}")
...
graph_idx_over_all = 462

concept_name = "concept_alignment_relations"
...
graph_idx_over_all, graphs_all_symmetric = alignment_relations(
    all_node_ids,
    nodes_shapes,
    nodes_colors,
...
graph_idx_over_all, graphs_all_non_symmetric = alignment_relations(
    all_node_ids,
    nodes_shapes,
    nodes_colors,
    nodes_with_noise_pos_dict,
...
dataset_pytorch_folder = os.path.join("data", "output", "concept_alignment_relations")
with open(os.path.join(dataset_pytorch_folder, 'concept_alignment_relations.pkl'), 'wb') as f:
